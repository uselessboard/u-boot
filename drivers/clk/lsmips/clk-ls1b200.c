...
#include <dm.h>
#include <dt-bindings/clock/ls1b200-clk.h>
...
static ulong ls1b200_clk_get_rate(struct clk *clk)
{
	struct ls1b200_clk_priv *priv = dev_get_priv(clk->dev);
	u32 val;

	if (clk->id >= ARRAY_SIZE(ls1b200_clks))
		return 0;

	switch (ls1b200_clks[clk->id]) {
	case CLK_SRC_CPU:
		return priv->cpu_clk;
	case CLK_SRC_CPU_D2:
	...
	case CLK_SRC_PERI:
		...
		if (val & PERI_CLK_FROM_XTAL_SEL)
			...
			return 40000000;
	default:
		return ls1b200_clks[clk->id];
	}
}

static int ls1b200_clk_enable(struct clk *clk)
{
	struct ls1b200_clk_priv *priv = dev_get_priv(clk->dev);
...
static int ls1b200_clk_disable(struct clk *clk)
{
	struct ls1b200_clk_priv *priv = dev_get_priv(clk->dev);
...
const struct clk_ops ls1b200_clk_ops = {
	.enable = ls1b200_clk_enable,
	.disable = ls1b200_clk_disable,
	.get_rate = ls1b200_clk_get_rate,
};

static int ls1b200_clk_probe(struct udevice *dev)
{
	struct ls1b200_clk_priv *priv = dev_get_priv(dev);
...
static const struct udevice_id ls1b200_clk_ids[] = {
	{ .compatible = "mediatek,ls1b200-clk" },
	{ }
};

U_BOOT_DRIVER(ls1b200_clk) = {
	.name = "ls1b200-clk",
	.id = UCLASS_CLK,
	.of_match = ls1b200_clk_ids,
	.probe = ls1b200_clk_probe,
	.priv_auto_alloc_size = sizeof(struct ls1b200_clk_priv),
	.ops = &ls1b200_clk_ops,
};
